...
ifeq ($(OS),win32)
    ...
    TEST_COMMAND := set PYTHONPATH=$(PYTHONPATH) && poetry run pytest -c pyproject.toml --cov-report=html --cov=pne tests/
else
	PYTHONPATH := `pwd`
    TEST_COMMAND := PYTHONPATH=$(PYTHONPATH) poetry run pytest -c pyproject.toml --cov-report=html --cov=pne tests/
endif
...
check-codestyle:
	...
	poetry run ruff check --config pyproject.toml .lint: test check-codestyle 
